...
function goToStep2() {
    ...
    if (!unityInstance) {
        ...
        // Unity已加载,直接发送数据并播放 - 传入整数
        const taxAmountInt = Math.round(currentCalculation.taxPrice);
        ...
        console.log('✅ 税款已发送并自动播放:', taxAmountInt);
    }
    ...
    createUnityInstance(document.querySelector("#unity-canvas"), config, (progress) => {
        ...
        if (currentCalculation.taxPrice > 0) {
            setTimeout(() => {
                ...
                unityInstance.SendMessage('MasterController', 'SetPrice', taxAmountInt.toString());
                unityInstance.SendMessage('MasterController', 'PlayAnimation');
                console.log('✅ Unity加载完成，自动发送数据并播放:', taxAmountInt);
            }, 500); // 等待Unity完全初始化
        }
    }).catch((message) => {
